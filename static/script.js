        ...
        cities.forEach(city => {
            ...
            div.onclick = async () => {
                ...
                if (nextSliderId && placesCount < MAX_PLACES) {
                    console.log(`Adding slider for Place ${placesCount + 1}`);
                    placesCount++;
...
function drawRoute(index) {
    ...
    const text = `${data.distance.toFixed(2)} ${translations[currentLanguage].km}, ${data.duration.toFixed(2)} ${translations[currentLanguage].hrs}`;
...
async function saveMap() {
    ...
    // Debugging: Log layers before saving
    console.log('Markers:', markers.length, 'Route Layers:', routeLayers.length, 'Label Layers:', labelLayers.length, 'Name Labels:', nameLabels.length);

    // Temporarily remove divIcon layers to avoid error
    ...
    // Ensure all other layers are on the map and fully rendered
    markers.forEach(marker => {
        if (marker && !map.hasLayer(marker)) marker.addTo(map);
    });
    routeLayers.forEach(layer => {
        if (layer && map.hasLayer(layer)) {
            map.removeLayer(layer); // Remove and re-add to force refresh
            layer.addTo(map);
        }
    });

    // Recompute bounds and force full re-render
    const allBounds = L.latLngBounds([
        ...markers.map(marker => marker.getLatLng()),
        ...routeLayers.map(layer => layer.getBounds().getCenter())
    ]);
    map.fitBounds(allBounds, { padding: [50, 50] }); // Add padding for visibility
    map.invalidateSize();
    map.eachLayer(layer => {
    ...
    // Debugging: Log map bounds, zoom, and route positions
    console.log('Map bounds:', map.getBounds().toBBoxString(), 'Zoom:', map.getZoom());
    routeLayers.forEach((layer, i) => {
        const midPoint = layer.getBounds().getCenter();
        const pixelPos = map.latLngToContainerPoint(midPoint);
        console.log(`Route ${i + 1} midpoint position: x=${pixelPos.x}, y=${pixelPos.y}`);
    });
    ...
    leafletImage(map, function(err, canvas) {
        ...
        // Draw route information on the canvas
        routeData.forEach((data, index) => {
            if (data && data.coordinates && data.coordinates.length > 0) {
                const midPointIndex = Math.floor(data.coordinates.length / 2);
                const midPointLatLng = [data.coordinates[midPointIndex][1], data.coordinates[midPointIndex][0]];
                const pixelPos = map.latLngToContainerPoint(midPointLatLng);
                const text = `${data.distance.toFixed(2)} ${translations[currentLanguage].km}, ${data.duration.toFixed(2)} ${translations[currentLanguage].hrs}`;
                const textWidth = text.length * 6;
                const textHeight = 20;

                ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
                ctx.fillRect(pixelPos.x - textWidth / 2, pixelPos.y - textHeight / 2, textWidth, textHeight);
                ctx.fillStyle = 'black';
                ctx.font = '11px Arial';
                ctx.textAlign = 'center';
                ctx.fillText(text, pixelPos.x, pixelPos.y + 4);
            }
        });
        ...
        ctx.fillStyle = 'white';
        ctx.fillRect(0, canvas.height - 55, canvas.width, 30);
        ctx.fillStyle = 'black';
        ctx.font = '16px Arial';
        ctx.textAlign = 'center';
        ctx.fillText(totalText, canvas.width / 2, canvas.height - 35);
...
function clearAll() {
    ...
    routeLayers.forEach(layer => layer && map.removeLayer(layer));
    labelLayers.forEach(layer => layer && map.removeLayer(layer));
    nameLabels.forEach(layer => layer && map.removeLayer(layer));
    markers = [];
    routeLayers = [];
    labelLayers = [];
    nameLabels = [];
